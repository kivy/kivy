...
cd SDL2_mixer-2.0.4;
./configure --prefix="$HOME/kivy_build" --bindir="$HOME/kivy_build/bin" --enable-music-mod-modplug-shared=no --enable-music-mod-mikmod-shared=no --enable-music-midi-fluidsynth-shared=no --enable-music-ogg-shared=no --enable-music-flac-shared=no --enable-music-mp3-mpg123-shared=no;
make;
make install;
make distclean;
...
cd SDL2_image-2.0.5;
./configure --prefix="$HOME/kivy_build" --bindir="$HOME/kivy_build/bin" --enable-png-shared=no --enable-jpg-shared=no --enable-tif-shared=no --enable-webp-shared=no;
make;
make install;
make distclean;
...
cd "$harf_root"
meson setup build --wrap-mode=default --buildtype=release -Dglib=disabled -Dgobject=disabled -Dcairo=disabled -Dfreetype=enabled --prefix="$HOME/kivy_build" --bindir="$HOME/kivy_build/bin" --libdir="$HOME/kivy_build/lib"
...
env CFLAGS="$(pkg-config --cflags harfbuzz)" LDFLAGS="$(pkg-config --libs harfbuzz)" ./configure --prefix="$HOME/kivy_build" --bindir="$HOME/kivy_build/bin"
make
make install
make distclean
...
for PYBIN in /opt/python/*3*/bin; do
    if [[ $PYBIN != *"34"* && $PYBIN != *"35"* ]]; then
        ...
        USE_HARFBUZZ=1 KIVY_SPLIT_EXAMPLES=1 USE_X11=1 USE_SDL2=1 USE_PANGOFT2=0 USE_GSTREAMER=0 "${PYBIN}/pip" wheel --no-deps . -w dist/;
    fi
done
