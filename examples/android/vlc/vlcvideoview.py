...
class VlcMediaPlayerEventsRedirector(PythonJavaClass):
    ...
    def __init__(self, host):
        ...
        self.eventTypesRedirectionMap = {
            ...
            # jVlcMediaPlayerEvent.Buffering:        ('buffering',
            # host.on_vlcplayer_buffering),
            ...
            # jVlcMediaPlayerEvent.Backward:         ('backward',
            # host.on_vlcplayer_backward),
            ...
            # jVlcMediaPlayerEvent.LengthChanged:    ('length',
            # host.on_vlcplayer_length),
            jVlcMediaPlayerEvent.Vout:             ('vout', host.on_vlcplayer_vout),
            # jVlcMediaPlayerEvent.ScrambleChanged:  ('scramble',
            # host.on_vlcplayer_scramble),
            ...
            # jVlcMediaPlayerEvent.ESSelected:       ('es_selected',
            # host.on_vlcplayer_es_selected),
        }
...
class VlcVOutEventsRedirector(PythonJavaClass):
    ...
    def onNewLayout(self, vlcVout, width, height, visibleWidth, visibleHeight, sarNum, sarDen):
        Logger.info("VlcVideoView: Vout new layout width={}, height={}, visibleWidth={}, visibleHeight={}, sarNum={}, sarDen={}".format(
        ...
        if self.options:
            ...
            for n, v in self.options.items():
                if n == 'hw-decoder':
                    ...
                    Logger.info(
                        "VlcVideoView: HWDecoder enabled {}, forced {}".format(hw_enable, hw_force))
                else:
...
#         self._mediaPlayer.setVideoTrack(-1);
    ...
    def _apply_aspect_ratio(self):
        ...
        Logger.info('VlcVideoView: aspect_ratio %.2f, sv_pos %d,%d  size %d,%d' % (
            aspect_ratio, left, top, width, height))
        ...
        if adjusted_width < width:
            ...
            Logger.info('VlcVideoView: aspect_ratio %.2f, decrease width to %d, changed left to %d' % (
                aspect_ratio, width, left))
        elif adjusted_height < height:
            left = 0
            top = int((height - adjusted_height) / 2)
            height = adjusted_height
            Logger.info('VlcVideoView: aspect_ratio %.2f, decrease height to %d, changed top to %d' % (
                aspect_ratio, adjusted_height, top))
        else:
            Logger.info('VlcVideoView: aspect_ratio %.2f, ignored %d %d to %d %d' % (
    ...
    def _apply_video_player_state(self):
        if not self._mediaPlayer:
            Logger.info(
                'VlcVideoView: waiting for Mediaplayer %s (%s)' % (self.source, self.state))
...
#             self._mediaPlayer.setVideoTrack(-1);
    ...
    def on_aspect_ratio(self, instance, aspect_ratio):
        Logger.info(
            'VlcVideoView: on_aspect_ratio {} for {}'.format(aspect_ratio, self.source))
        self._apply_aspect_ratio()
    ...
    def on_vlc_hw_error(self, mediaPlayerEvent):
        ...
        Logger.info(
            'VlcVideoView: on_vlc_hw_error {} for {}'.format(errorMessage, self.source))
    ...
    def on_vlcplayer_end_reached(self, mediaPlayerEvent):
        self.eos = True
        Logger.info(
            'VlcVideoView: on_vlcplayer_end_reached for {}'.format(self.source))

    def on_vlcplayer_error(self, mediaPlayerEvent):
        errorMessage = jVlcUtil.getErrorMsg()
        Logger.info(
            'VlcVideoView: on_vlcplayer_error {} for {}'.format(errorMessage, self.source))
        self.player_state = 'error'
        self.eos = True
    ...
    def on_vlcplayer_position(self, mediaPlayerEvent):
        ...
        Logger.info('VlcVideoView: on_vlc_position {} for {} ({})'.format(
            self.position_normalized, self.source, self.state))
    ...
    def on_vlcmedia_duration_changed(self, mediaEvent):
        ...
        Logger.info('VlcVideoView: on_vlcmedia_duration_changed {} for {}'.format(
            self.duration, self.source))
        self._log_media_info()
    ...
    def _log_media_tracks(self):
        for trackN in xrange(0, self._mediaObj.getTrackCount()):
            ...
            Logger.info('VlcVideoView: Track[{}].type: {} ({}), id: {}, codec: {}, bitrate: {}, lang: {}, descr: {}'.format(
                        trackN, self.TrackTypeNames.get(
