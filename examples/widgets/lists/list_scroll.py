...
(Just so you know, the next line is very long, with the girl name data, in case
        it flakes out your editor).
...
class GirlNameDataItem(SelectableDataItem):
    def __init__(self, **kwargs):
        super(GirlNameDataItem, self).__init__(**kwargs)
...
<ScrollTestView>:
    ...
    ListView:
        ...
        size_hint: .2, 1.0
        DataBinding:
            source: app.us_baby_girl_names_controller
            prop: 'data'
''')
...
class ScrollTestApp(App):
    ...
    def __init__(self, **kwargs):
        ...
        self.list_item_class_args = \
                lambda row_index, item: {'text': "{0} ({1})".format(item.name, row_index),
                                         'size_hint_y': None,
                                         'height': 25}

        self.us_baby_girl_names_controller = \
                ListController(
                    data=[GirlNameDataItem(name=g) for g in girl_names_2012_usa],
