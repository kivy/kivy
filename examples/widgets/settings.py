...
class SettingsApp(App):
    ...
    def build(self):
        ...
        sidebar_button.bind(
            on_press=lambda j: self.set_settings_cls(SettingsWithSidebar)
        )
        spinner_button = Button(text="Spinner")
        spinner_button.bind(
            on_press=lambda j: self.set_settings_cls(SettingsWithSpinner)
        )
        tabbed_button = Button(text="TabbedPanel")
        tabbed_button.bind(
            on_press=lambda j: self.set_settings_cls(SettingsWithTabbedPanel)
        )
    ...
    def display_settings(self, settings):
        if self.display_type == "popup":
            ...
            if p is None:
                self.settings_popup = p = Popup(
                    content=settings, title="Settings", size_hint=(0.8, 0.8)
                )
