...
def require(version):
    ...
    if tag and not systag:
        Logger.warning(
            "Application requested a dev version of Kivy. "
    ...
    if sysrevision < revision:
        raise Exception(
            "The version of Kivy installed on this system "
            "is too old. "
            "(You have %s, but the application requires %s)"
            % (__version__, version)
        )
...
def get_includes():
    ...
    return [
        join(root_dir, "graphics"),
        join(root_dir, "tools", "gles_compat"),
...
kivy_binary_deps_dir = environ.get(
    "KIVY_BINARY_DEPS", join(kivy_base_dir, "binary_deps")
)
...
for importer, modname, package in _packages:
    try:
        ...
        _logging_msgs.append(
            'deps: Successfully imported "{}.{}"{}'.format(
                package, modname, version
            )
        )
    except ImportError as e:
        Logger.warning(
            'deps: Error importing dependency "{}.{}": {}'.format(
                package, modname, str(e)
            )
        )
...
if not environ.get("KIVY_DOC_INCLUDE"):
    ...
    if (
        "KIVY_UNITTEST" not in environ
        and "KIVY_PACKAGING" not in environ
        and "KIVY_NO_ARGS" not in environ
    ):
    ...
    if need_save and "KIVY_NO_CONFIG" not in environ:
        try:
            ...
            Logger.exception(
                "Core: error while saving default" "configuration file:",
                str(e),
            )
