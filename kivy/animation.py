...
class Animation(EventDispatcher):
    ...
    def _update(self, dt):
        ...
        for uid in list(widgets.keys()):
            ...
            if self._duration:
                progress = min(1.0, anim["time"] / self._duration)
            else:
                progress = 1
...
class CompoundAnimation(Animation):
    def stop_property(self, widget, prop):
        ...
        if not self.anim1.have_properties_to_animate(
            widget
        ) and not self.anim2.have_properties_to_animate(widget):
            self.stop(widget)
    ...
    def have_properties_to_animate(self, widget):
        return self.anim1.have_properties_to_animate(
            widget
        ) or self.anim2.have_properties_to_animate(widget)

    @property
    def animated_properties(self):
        return ChainMap(
            {}, self.anim2.animated_properties, self.anim1.animated_properties
        )
...
class Sequence(CompoundAnimation):
    def __init__(self, anim1, anim2):
        ...
        self.anim1.bind(
            on_complete=self.on_anim1_complete, on_progress=self.on_anim1_progress
        )
        self.anim2.bind(
            on_complete=self.on_anim2_complete, on_progress=self.on_anim2_progress
        )
