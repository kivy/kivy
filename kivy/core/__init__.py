...
def core_select_lib(
    category, llist, create_instance=False, base="kivy.core", basemodule=None
):
    ...
    for option, modulename, classname in llist:
        try:
            ...
            # ok !
            Logger.info(
                "{0}: Provider: {1}{2}".format(
                    category.capitalize(),
                    option,
                    "({0} ignored)".format(libs_ignored)
                    if libs_ignored
                    else "",
                )
            )
            ...
            libs_ignored.append(modulename)
            Logger.debug(
                "{0}: Ignored <{1}> (import error)".format(
                    category.capitalize(), option
                )
            )
            ...
            errs.append((option, e, sys.exc_info()[2]))
            Logger.error(
                "{0}: Unable to use {1}".format(category.capitalize(), option)
            )
            Logger.error(
    ...
    err = "\n".join(
        [
            "{} - {}: {}\n{}".format(
                opt, e.__class__.__name__, e, "".join(traceback.format_tb(tb))
            )
...
def core_register_libs(category, libs, base="kivy.core"):
    ...
    for option, lib in libs:
        ...
        if option not in kivy_options:
            Logger.debug(
                "{0}: option <{1}> ignored by config".format(
                    category.capitalize(), option
                )
            )
    ...
    for item in kivy_options:
        try:
            ...
            try:
                ...
                continue
            __import__(
                name="{2}.{0}.{1}".format(category, lib, base),
                globals=globals(),
                locals=locals(),
                fromlist=[lib],
                level=0,
            ...
            Logger.trace(
                "{0}: Unable to use <{1}> as loader!".format(
                    category.capitalize(), option
                )
            )
            Logger.trace("", exc_info=e)
    ...
    Logger.info(
        "{0}: Providers: {1} {2}".format(
            category.capitalize(),
            ", ".join(libs_loaded),
            "({0} ignored)".format(", ".join(libs_ignored))
            if libs_ignored
            else "",
        )
    )
...
def handle_win_lib_import_error(category, provider, mod_name):
    ...
    if not os.path.exists(depends_bin):
        raise ValueError(
            '"{}" provided in {} does not exist'.format(depends_bin, env_var)
        )
