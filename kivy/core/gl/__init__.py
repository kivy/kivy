...
def msgbox(message):
    if sys.platform == "win32":
        ...
        ctypes.windll.user32.MessageBoxW(
            None, LPCWSTR(message), "Kivy Fatal Error", 0
        )
        sys.exit(1)
    ...
    def print_gl_version():
        ...
        if (major, minor) < MIN_REQUIRED_GL_VERSION and backend != "mock":
            if hasattr(sys, "_kivy_opengl_required_func"):
                sys._kivy_opengl_required_func(
                ...
                ).format(major, minor, version, vendor, renderer)
                Logger.critical(msg)
                msgbox(msg)
        ...
        if platform != "android":
            ...
            Logger.info(
                "GL: Shading version <{0}>".format(
                    glGetString(GL_SHADING_LANGUAGE_VERSION)
                )
            )
        Logger.info(
            "GL: Texture max size <{0}>".format(
                glGetIntegerv(GL_MAX_TEXTURE_SIZE)[0]
            )
        )
        Logger.info(
            "GL: Texture max units <{0}>".format(
