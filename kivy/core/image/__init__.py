...
class ImageData(object):
    ...
    def __repr__(self):
        return (
            "<ImageData width=%d height=%d fmt=%s "
...
class ImageLoaderBase(object):
    ...
    def populate(self):
        ...
        if __debug__:
            Logger.trace(
                "Image: %r, populate to textures (%d)"
        ...
        for count in range(len(self._data)):
            ...
            # cache
            chr = type(fname)
            uid = chr("%s|%d|%d") % (fname, self._mipmap, count)
        ...
        for zfilename in znamelist:
            try:
                ...
                for loader in ImageLoader.loaders:
                    if (
                        ext not in loader.extensions()
                        or not loader.can_load_memory()
                    ):
                        continue
                    Logger.debug(
                        "Image%s: Load <%s> from <%s>"
        ...
        if ext == "zip":
            ...
            for loader in ImageLoader.loaders:
                if ext not in loader.extensions():
                    continue
                Logger.debug(
                    "Image%s: Load <%s>" % (loader.__name__[11:], filename)
                )
...
class Image(EventDispatcher):
    ...
    def __init__(self, arg, **kwargs):
        ...
        self._anim_index = 0
        self._anim_delay = 0
        self.anim_delay = kwargs.get("anim_delay", 0.25)
    ...
    def _set_anim_delay(self, x):
        ...
        if self._anim_available:
            ...
            if self._anim_delay >= 0:
                self._anim_ev = Clock.schedule_interval(
                    self._anim, self._anim_delay
                )
    ...
    image = property(
        _get_image, _set_image, doc="Get/set the data image object"
    )
    ...
    filename = property(
        _get_filename, _set_filename, doc="Get/set the filename of image"
    )
    ...
    def load_memory(self, data, ext, filename="__inline__"):
        ...
        loaders = [
            loader
            for loader in ImageLoader.loaders
            if loader.can_load_memory() and ext in loader.extensions()
        ]
        if not loaders:
    ...
    def read_pixel(self, x, y):
        ...
        if data.data is None:
            raise EOFError(
                "Image data is missing, make sure that image is"
...
image_libs += [("ffpy", "img_ffpyplayer"), ("pil", "img_pil")]
