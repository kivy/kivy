"""
System tray support for Kivy applications.
"""
...
cdef class _SDLPointerHandler:
    ...
    cdef void set_pointer(self, uintptr_t value):
        ...
        if self.__pointer != 0 and value != 0 and self.__pointer != value:
            Logger.warning("SystemTray: Overwriting an existing active SDL pointer. Ensure items are not shared across multiple active menus.")
...
cdef class TrayMenuItem(_SDLPointerHandler):
    ...
    * **submenu**: Item that opens another menu when hovered/clicked
    ...
    def label(self, value):
        if self.__type == 'separator':
            raise ValueError("Separator items cannot have a label.")
    ...
    def enabled(self, value):
        if self.__type == 'separator':
            raise ValueError("Separator items cannot be enabled or disabled.")
    ...
    def checked(self, value):
        if value and self.__type != 'checkbox':
            raise ValueError(f"'{self.__type}' items cannot be checked")
    ...
    def callback(self, value):
        if value is not None and self.__type in ('separator', 'submenu'):
            raise ValueError(f"'{self.__type}' items cannot have callbacks.")
...
cdef class TrayMenu(_SDLPointerHandler):
    ...
    def __init__(self, list items = None) -> None:
        """
        Initialize a tray menu with optional items.
        
        Args:
            items: List of TrayMenuItem objects to initialize the menu with.
        ...
        Examples:
            ...
            >>> 
            >>> items = [TrayMenuItem("File"), TrayMenuItem("Edit")]
            >>> menu = TrayMenu(items)
    ...
    cpdef void add_items(self, list items):
        """
        Add multiple items to the menu at once.
        
        Args:
            items (list): List of TrayMenuItem objects to add
        """
        for item in items:
            self.add_item(item)
