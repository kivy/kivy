...
class WindowBase(EventDispatcher):
    ...
    size = AliasProperty(
        _get_size,
        _set_size,
        bind=("_size", "_rotation", "softinput_mode", "keyboard_height"),
    )
    ...
    clearcolor = AliasProperty(
        _get_clearcolor, _set_clearcolor, bind=("_clearcolor",)
    )
    ...
    height = AliasProperty(
        _get_height,
        None,
        bind=("_rotation", "_size", "softinput_mode", "keyboard_height"),
    )
    """Rotated window height.
    ...
    center = AliasProperty(
        _get_center, bind=("width", "height", "_density"), cache=True
    )
    ...
    system_size = AliasProperty(
        _get_system_size,
        _set_system_size,
        bind=("_size", "softinput_mode", "keyboard_height"),
        cache=True,
    )
    ...
    def __init__(self, **kwargs):
        ...
        if "minimum_height" not in kwargs:
            kwargs["minimum_height"] = Config.getint(
                "graphics", "minimum_height"
            )
        ...
        if "position" not in kwargs:
            kwargs["position"] = Config.getdefault(
        ...
        self.bind(
            size=self.trigger_keyboard_height,
            rotation=self.trigger_keyboard_height,
        )

        self.bind(
            softinput_mode=lambda *dt: self.update_viewport(),
    ...
    def maximize(self):
        ...
        """
        Logger.warning(
            "Window: maximize() is not implemented in the current "
            "window provider."
        )
    ...
    def minimize(self):
        ...
        """
        Logger.warning(
            "Window: minimize() is not implemented in the current "
            "window provider."
        )
    ...
    def hide(self):
        ...
        """
        Logger.warning(
            "Window: hide() is not implemented in the current "
            "window provider."
        )
    ...
    def show(self):
        ...
        """
        Logger.warning(
            "Window: show() is not implemented in the current "
            "window provider."
        )
    ...
    def raise_window(self):
        ...
        """
        Logger.warning(
            "Window: raise_window is not implemented in the current"
            " window provider."
        )
    ...
    def get_gl_backend_name(self):
        ...
        return cgl_get_backend_name(
            allowed=self.gl_backends_allowed, ignored=self.gl_backends_ignored
        )
    ...
    if Config:
        ...
        def __exit(section, name, value):
            WindowBase.__dict__[
                "on_keyboard"
            ].exit_on_escape = Config.getboolean("kivy", "exit_on_escape")
