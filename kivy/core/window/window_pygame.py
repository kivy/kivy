...
class WindowPygame(WindowBase):
    ...
    def create_window(self, *largs):
        ...
        if platform in ("linux", "macosx", "win") and Config.getboolean(
            "graphics", "resizable"
        ):
        ...
        if multisamples > 0:
            ...
            pygame.display.gl_set_attribute(
                pygame.GL_MULTISAMPLESAMPLES, multisamples
            )
        ...
        if self.position == "auto":
            ...
            raise ValueError(
                "position token in configuration accept only "
                '"auto" or "custom"'
            )
        ...
        try:
            ...
            if multisamples:
                Logger.warning(
                    "WinPygame: Video: failed (multisamples=%d)" % multisamples
                )
        ...
        Logger.debug(
            "Window: Actual window size: %dx%d", info.current_w, info.current_h
        )
        if platform != "android":
