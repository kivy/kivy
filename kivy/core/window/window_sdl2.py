...
class WindowSDL(WindowBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(
            minimum_width=self._set_minimum_size,
    ...
    def create_window(self, *largs):
        ...
        if not self.initialized:
            ...
            state = (
                Config.get("graphics", "window_state")
                if self._is_desktop
                else None
            )
    ...
    def _set_shape(
        self, shape_image, mode="default", cutoff=False, color_key=None
    ):
        modes = ("default", "binalpha", "reversebinalpha", "colorkey")
        color_key = color_key or (0, 0, 0, 1)
        if mode not in modes:
            Logger.warning(
                "Window: shape mode can be only " "{}".format(", ".join(modes))
            )
            return
    ...
    def mainloop(self):
        ...
        while True:
            ...
            # video resize
                ...
                if self.dispatch(
                    "on_key_down", key, scancode, kstr, self.modifiers
                ):
                    continue
                self.dispatch(
                    "on_keyboard", key, scancode, kstr, self.modifiers
                )
