...
class GestureStroke:
    ...
    def normalize_stroke(self, sample_points=32):
        ...
        target_stroke_size = self.stroke_length(self.points) / float(
            sample_points
        )
        new_points = list()
        ...
        for curr in self.points[1:]:
            ...
            if d > 0:
                ...
                while dst_distance < src_distance:
                    ...
                    dst_distance = (
                        self.stroke_length(self.points)
                        / float(sample_points)
                        * len(new_points)
                    )
        ...
        if not len(new_points) == sample_points:
            raise ValueError(
                "Invalid number of strokes points; got "
                "%d while it should be %d" % (len(new_points), sample_points)
            )
...
class Gesture:
        ...
        if isinstance(point_list, list) or isinstance(point_list, tuple):
            for point in point_list:
                if isinstance(point, GesturePoint):
                    ...
                    raise TypeError(
                        "The point list should either be "
    ...
    def dot_product(self, comparison_gesture):
        ...
        if len(comparison_gesture.strokes) != len(self.strokes):
            return -1
        if (
            getattr(comparison_gesture, "gesture_product", True) is False
            or getattr(self, "gesture_product", True) is False
        ):
            return -1
        dot_product = 0.0
        for stroke_index, (my_stroke, cmp_stroke) in enumerate(
            ...
            for pt_index, (my_point, cmp_point) in enumerate(
                ...
                dot_product += (
                    my_point.x * cmp_point.x + my_point.y * cmp_point.y
                )
        return dot_product
    ...
    def __eq__(self, comparison_gesture):
        ...
        if isinstance(comparison_gesture, Gesture):
            ...
            if score > (1.0 - self.tolerance) and score < (
                1.0 + self.tolerance
            ):
                return True
            else:
