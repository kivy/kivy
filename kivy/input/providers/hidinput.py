    ...
    class HIDInputMotionEventProvider(MotionEventProvider):
        ...
        def __init__(self, device, args):
            ...
            if "rotation" not in self.default_ranges:
                ...
                Logger.error(
                    "HIDInput: invalid rotation value ({})".format(
        ...
        def _thread_run(self, **kwargs):
                ...
                if ev_type == EV_SYN:
                    ...
                    if ev_code == ABS_MT_TRACKING_ID:
                        ...
                        point["pressure"] = normalize(
                            ev_value, range_min_pressure, range_max_pressure
                        )
            ...
            def process_as_mouse_or_keyboard(
                ...
                if ev_type == EV_SYN:
                    if ev_code == SYN_REPORT:
                        process([point])
                        if "button" in point and point["button"].startswith(
                            "scroll"
                        ):
                    ...
                    if ev_code == 0:
                        assign_rel_coord(
                            point,
                            min(1.0, max(-1.0, ev_value / 1000.0)),
                            invert_x,
                            "xy",
                        )
                    elif ev_code == 1:
                    ...
                    if ev_code == ABS_X:
                        val = normalize(
                            ev_value, range_min_abs_x, range_max_abs_x
                        )
                        assign_coord(point, val, invert_x, "xy")
                    elif ev_code == ABS_Y:
                        val = 1.0 - normalize(
                            ev_value, range_min_abs_y, range_max_abs_y
                        )
                        assign_coord(point, val, invert_y, "yx")
                    ...
                    if ev_code in buttons.keys():
                        ...
                        if ev_code not in keyboard_keys:
                            Logger.warn(
                                "HIDInput: unhandled HID code: {}".format(
                                    ev_code
                                )
                            )
                            return
                        ...
                        z = keyboard_keys[ev_code][
                            -1 if "shift" in Window._modifiers else 0
                        ]
                        if z.lower() not in Keyboard.keycodes:
                            # or if it is not in this LUT
                            Logger.warn(
                                "HIDInput: unhandled character: {}".format(z)
                            )
                            return
            ...
            device_name = (
                fcntl.ioctl(fd, EVIOCGNAME + (256 << 16), " " * 256)
                .decode()
                .strip()
            )
            ...
            for x in range(EV_MAX):
                ...
                sbit = fcntl.ioctl(
                    fd, EVIOCGBIT + x + (KEY_MAX << 16), " " * sz_l
                )
                ...
                for y in range(KEY_MAX):
                    ...
                    absinfo = fcntl.ioctl(
                        fd,
                        EVIOCGABS + y + (struct_input_absinfo_sz << 16),
