...
devices = {
    ...
    "tablet_huawei_mediapad_m3_lite_10": (
...
def apply_device(device, scale, orientation):
    ...
    Logger.info(
        "Screen: size={0}x{1} dpi={2} density={3} "
...
def usage(device=None):
    if device:
        Logger.error("Screen: The specified device ({0}) is unknown.", device)
    ...
    for device, info in devices.items():
        print(
            "{0:12} {1:<22} {2:<8} {3:<8} {4:<5} {5:<8}".format(device, *info)
        )
    print("\n")
