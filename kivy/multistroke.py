...
class Recognizer(EventDispatcher):
    ...
    def import_gesture(self, data=None, filename=None, **kwargs):
        ...
        This method accepts optional :meth:`Recognizer.filter` arguments.
        If none are specified, then all gestures in the specified data are
        imported.

        :Arguments:

            `data`
            A base64-encoded string representing the gesture data to be
            imported. This is typically generated by :meth:`export_gesture`.

            `filename`
            The name of the file containing the gesture data to be
            imported. If specified, the data will be read from this file.

        :Raises:
            `MultistrokeError`
            If neither `data` nor `filename` is specified.
        '''
        ...
        strokes = [gesture.strokes for gesture in new]
        cand = self._candidate(strokes)
        result = ProgressTracker(cand, len(new))

        # This callback is scheduled once per frame until completed
        def _compare_data(goodscore=None, timeout=0, delay=0):
            start_gc = result._completed
            stop_now = False

            while not stop_now and (new and not result._break_flag) and \
            ...
            if not new:
                ...
                Clock.schedule_once(_compare_data, delay)
                return True
