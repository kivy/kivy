...
def requires_graphics(func):
    if "mock" == cgl_get_backend_name():
        return pytest.mark.skip(
            reason="Skipping because gl backend is set to mock"
        )(func)
    return func
...
class GraphicUnitTest(_base):
        ...
        try:
            ...
            if not exists(reffn):
                ...
                if sd1 != sd2:
                    ...
                    if self.interactive_ask_diff(
                        sourcecodeask, tmpfn, reffn, self.id()
                    ):
    ...
    def interactive_ask_diff(self, code, tmpfn, reffn, testid):
        ...
        Label(
            ...
            "image as the reference one.." % testid,
        ).pack()
        Label(root, text="Which one is good ?").pack()
        Label(root, text=code, justify=LEFT).pack(side=RIGHT)
        Label(root, image=phototmp).pack(side=RIGHT)
        Label(root, image=photoref).pack(side=LEFT)
        Button(root, text="Use the new image -->", command=do_yes).pack(
            side=BOTTOM
        )
        Button(root, text="<-- Use the reference", command=do_close).pack(
            side=BOTTOM
        )
        root.mainloop()
    ...
    def inner_func(func):
        ...
        if app_cls_func is not None:
            func = pytest.mark.parametrize(
                "kivy_app", [[app_cls_func],], indirect=True,
            )(func)
