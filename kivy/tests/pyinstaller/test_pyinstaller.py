...
class PyinstallerBase(object):
    ...
    def teardown_class(cls):
        shutil.rmtree(
            os.path.join(cls.pinstall_path, "__pycache__"), ignore_errors=True
        )
        shutil.rmtree(
...
class TestVideoWidget(PyinstallerBase):
    ...
    def get_env(cls):
        ...
        env["__KIVY_VIDEO_TEST_FNAME"] = os.path.abspath(
            os.path.join(kivy.kivy_examples_dir, "widgets", "cityCC0.mpg")
        )
        return env
