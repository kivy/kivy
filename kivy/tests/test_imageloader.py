...
def match_prediction(pixels, fmt, fd, pitch):
    ...
    if len(pixels) != pitch * fd["h"]:
        fail(
            "Pitch errror: pitch {} * {} height != {} pixelbytes".format(
                pitch, fd["h"], len(pixels)
            )
        )
    ...
    for char in fd["pattern"]:
        ...
        if len(pix) != bpp:
            fail(
                "Want {} bytes per pixel, got {}: {}".format(bpp, len(pix), pix)
            )
            break
        ...
        if char == "t":
            if get_pixel_alpha(pix, fmt) != 0:
                fail(
                    "pixel {} nonzero 't' pixel alpha {:02X}: {}".format(
            ...
            if not predict or not pix or predict != pix:
                fail(
                    "pixel {} {} format mismatch: want {} ({}) -- got {}".format(
                        pixnum, fmt, predict, char, pix
                    )
                )
        ...
        if pitchalign and (pixnum + 1) % fd["w"] == 0:
            ...
            if check != [0] * pitchalign:
                fail(
                    "Want {} 0x00 pitch align pixnum={}, pos={} got: {}".format(
                        pitchalign, pixnum, ptr + bpp, check
                    )
                )
            ptr += pitchalign
        ptr += bpp
    ...
    if ptr != len(pixels):
        fail(
            "Excess data: pixnum={} ptr={} bytes={}, bpp={} pitchalign={}".format(
                pixnum, ptr, len(pixels), bpp, pitchalign
            )
        )
...
class _TestContext(object):
    ...
    def dbg(self, msgtype, info):
        ...
        if DEBUG:
            print(
                "{} {} {}: {}".format(
                    self.loadercls.__name__, msgtype, self._fn, info
                )
            )
...
@unittest.skipIf(
    not os.path.isdir(asset(ASSETDIR)),
...
class ImageLoaderTestCase(unittest.TestCase):
    ...
    def _test_imageloader(self, loadercls, extensions=None):
        ...
        if fail:
            self.fail(
                "{}: {} passed, {} skipped, {} failed".format(
                    loadercls.__name__, ok, skip, fail
                )
            )
        return ctx
    ...
    def _test_image(self, fd, ctx, loadercls, imgdata):
        ...
        if pitch == 0 and bytes_per_pixel(fmt) * w * h != len(pixels):
            ctx.dbg(
                "PITCH",
                "pitch=0, expected fmt={} to be "
                "unaligned @ ({}bpp) = {} bytes, got {}".format(
                    fmt,
                    bytes_per_pixel(fmt),
            ...
            ctx.dbg(
                "PITCH",
                "fmt={}, pitch={}, expected {}".format(fmt, pitch, want_pitch),
            )
        ...
        if not success:
            if not msgs:
                ...
                for m in msgs:
                    ctx.dbg("PREDICT", m)
                ctx.fail(
                    "{} errors, see debug output: {}".format(
                        len(msgs), msgs[-1]
                    )
                )
            debug()
