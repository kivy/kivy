...
from kivy.input.providers.mouse import MouseMotionEventProvider as Mouse
...
class MultitouchSimulatorTestCase(GraphicUnitTest):
    ...
    def multitouch_dot_touch(self, button, **kwargs):
        ...
        if "on_demand" in kwargs and "scatter" not in kwargs:
            ...
            self.assertIn("multitouch_sim", mouse.touches["mouse1"].profile)
            self.assertTrue(mouse.multitouch_on_demand)
        ...
        dot_proxy = mouse.touches["mouse1"].ud.get("_drawelement")[1].proxy_ref
        ...
        win.dispatch(
            "on_mouse_up", 10, self.correct_y(win, 10), button, {}
        )  # no matter where
    ...
    def multitouch_dot_move(self, button, **kwargs):
        ...
        # the touch is released (in on_mouse_release)
            ...
            win.dispatch(
                "on_mouse_up", 10, self.correct_y(win, 10), "right", {}
            )
            color = mouse.touches["mouse1"].ud.get("_drawelement")[0].proxy_ref
            ellipse = (
                mouse.touches["mouse1"].ud.get("_drawelement")[1].proxy_ref
            )
            win.dispatch(
                "on_mouse_down", 10, self.correct_y(win, 10), "right", {}
            )
        ...
        self.assertEqual(
            ellipse.pos, (1, self.correct_y(win, win.height - 1))
        ...
        self.assertEqual(
            ellipse.pos, (40, self.correct_y(win, win.height - 40))
        )  # bounding box from Rectangle, R=10 -> 20 width
    ...
    def test_multitouch_appear(self):
        ...
        # do NOT make any hard refs to '_drawelement'
        color = mouse.touches["mouse1"].ud.get("_drawelement")[0].proxy_ref
        ellipse = mouse.touches["mouse1"].ud.get("_drawelement")[1].proxy_ref
        ...
        self.assertAlmostEqual(
            ellipse.pos[1], self.correct_y(win, win.height), delta=0.0001
        )
