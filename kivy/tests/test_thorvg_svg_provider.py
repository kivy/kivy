...
class TestParseUriParams:
    """Pure unit tests for the URI parameter parser."""
...
class TestProviderUriRegex:
    """Validate the updated _provider_uri_re with optional [params] block."""

    @pytest.fixture(autouse=True)
    def import_re(self):
        from kivy.core.image import _provider_uri_re
        self.re = _provider_uri_re

    def test_basic_uri_no_params(self):
        m = self.re.match('@image_provider:pil(image.png)')
        assert m is not None
        assert m.group(1) == 'pil'
        assert m.group(2) is None
        assert m.group(3) == 'image.png'

    def test_uri_with_size_param(self):
        m = self.re.match('@image_provider:thorvg_svg[size=256](icon.svg)')
        assert m is not None
        assert m.group(1) == 'thorvg_svg'
        assert m.group(2) == 'size=256'
        assert m.group(3) == 'icon.svg'

    def test_uri_with_multiple_params(self):
        m = self.re.match('@image_provider:thorvg_svg[size=512,mode=fit](a.svg)')
        assert m is not None
        assert m.group(2) == 'size=512,mode=fit'

    def test_uri_with_absolute_path(self):
        m = self.re.match('@image_provider:sdl3(/abs/path/image.jpg)')
        assert m is not None
        assert m.group(3) == '/abs/path/image.jpg'

    def test_uri_with_relative_path(self):
        m = self.re.match('@image_provider:imageio(../imgs/icon.png)')
        assert m is not None
        assert m.group(3) == '../imgs/icon.png'

    def test_plain_path_does_not_match(self):
        assert self.re.match('image.png') is None

    def test_wrong_prefix_does_not_match(self):
        assert self.re.match('@provider:pil(image.png)') is None

    def test_missing_provider_name_does_not_match(self):
        assert self.re.match('@image_provider:(image.png)') is None

    def test_missing_path_does_not_match(self):
        assert self.re.match('@image_provider:pil') is None
