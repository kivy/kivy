    ...
    for t in os.walk(dir_name):
        for filename in t[2]:
            if filename.endswith(".png"):
                ...
                if m is None:
                    yield {
                        "error": "png filename not following screenshot"
...
def iter_docstring_info(dir_name):
    ...
    for file_info in iter_filename_info(dir_name):
        ...
        if not os.path.exists(source):
            Logger.error(
                "Screen shot references source code that does "
                "not exist:  %s",
                source,
            )
            continue
    ...
    def t(left="", right=""):
        ...
        for info in infos:
            ...
            if len(l) > width1 or len(r) > width2:
                Logger.error(
                    'items to wide for generated table: "%s" and "%s"', l, r
                )
                return
            output.append(
                "| {0:{w1}} | {1:{w2}} |".format(l, r, w1=width1, w2=width2)
            )
...
def write_all_rst_pages():
    ...
    for info in infos:
        ...
        detail_name = slash(
            generation_dir, "gen__{}.rst".format(info["dunder"])
        )
