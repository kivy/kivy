...
def get_cython_versions(setup_cfg=""):
    ...
    cython_requires = "cython>={min_version},<={max_version},{exclusion}".format(
...
def get_cython_msg():
    ...
    at all: {}""".format(
        ", ".join(map(str, cython_unsupported))
    )
    ...
    install Cython via pip:
        ...
        pip install -I "{3}"{2}
    """.format(
            cython_min, cython_max, cython_unsupported_append, cython_requires
        )
    )
