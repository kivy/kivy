...
class DragBehavior(object):
    ...
    def on_touch_down(self, touch):
        ...
        if (
            self._drag_touch
            or ("button" in touch.profile and touch.button.startswith("scroll"))
            or not ((xx < x <= xx + w) and (yy < y <= yy + h))
        ):
            return super(DragBehavior, self).on_touch_down(touch)
    ...
    def on_touch_move(self, touch):
        if (
            ...
            or self._drag_touch is not touch
        ):
            return (
                super(DragBehavior, self).on_touch_move(touch)
                or self._get_uid() in touch.ud
            )
