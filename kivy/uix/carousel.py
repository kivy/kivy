...
class Carousel(StencilView):
    ...
    direction = OptionProperty(
        "right", options=("right", "left", "top", "bottom")
    )
    ...
    index = AliasProperty(
        _get_index, _set_index, bind=("_index", "slides"), cache=True
    )
    ...
    previous_slide = AliasProperty(
        _prev_slide, bind=("slides", "index", "loop"), cache=True
    )
    ...
    current_slide = AliasProperty(
        _curr_slide, bind=("slides", "index"), cache=True
    )
    ...
    next_slide = AliasProperty(
        _next_slide, bind=("slides", "index", "loop"), cache=True
    )
    ...
    def on__offset(self, *args):
        ...
        # Move to next slide?
        if (
            (direction == "r" and _offset <= -width)
            or (direction == "l" and _offset >= width)
            or (direction == "t" and _offset <= -height)
            or (direction == "b" and _offset >= height)
        ):
            if self.next_slide:
                self.index += 1
