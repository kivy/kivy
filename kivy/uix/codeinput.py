...
class CodeInput(CodeNavigationBehavior, TextInput):
    ...
    def _get_text_width(self, text, tab_width, _label_cached):
        ...
        cid = "{}\0{}\0{}".format(text, self.password, self._get_line_options())
    ...
    def _get_bbcode(self, ntext):
        ...
        try:
            ...
            ntext = ntext.replace("[", "\x01").replace("]", "\x02")
            ntext = highlight(ntext, self.lexer, self.formatter)
            ntext = ntext.replace("\x01", "&bl;").replace("\x02", "&br;")
            # replace special chars with &bl; and &br;
            ntext = "".join(
                ("[color=", str(self.text_color), "]", ntext, "[/color]")
            )
            ntext = ntext.replace("\n", "")
