...
class LottieWidget(Widget):
    ...
    * ``'pause'`` - halt and remain on the last frame.  Use this for
    ...
    def _on_provider_eos(self, provider, *largs):
        if provider is not self._lottie:
            return
        # For ``eos`` ``'stop'`` the provider will halt after this dispatch; mirror
        # that on :attr:`state` so the next ``state = 'play'`` is observed and
        # :meth:`on_state` runs (otherwise ``state`` would stay ``'play'`` and
        # playback would not restart).
        if self.eos == 'stop':
            self.state = 'stop'
