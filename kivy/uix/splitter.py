...
class Splitter(BoxLayout):
    ...
    sizable_from = OptionProperty(
        "left", options=("left", "right", "top", "bottom")
    )
    ...
    _parent_proportion = NumericProperty(0.0)
        ...
        if sz_frm in ("t", "b"):
            ...
            if self.keep_within_parent:
                if sz_frm == "l" and (self_x + diff_x) < self.parent.x:
                    ...
                    sz_frm == "r" and (self_right + diff_x) > self.parent.right
                ):
...
if __name__ == "__main__":
    ...
    class SplitterApp(App):
        def build(self):
            ...
            bx2.add_widget(Button())
            spl = Splitter(
                size_hint=(1, 0.25), pos_hint={"top": 1}, sizable_from="bottom"
            )
