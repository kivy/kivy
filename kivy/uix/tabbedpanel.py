...
class TabbedPanel(GridLayout):
    ...
    def set_def_tab(self, new_tab):
        if not issubclass(new_tab.__class__, TabbedPanelHeader):
            raise TabbedPanelException(
                "`default_tab_class` should be\
    ...
    default_tab = AliasProperty(
        get_def_tab, set_def_tab, bind=("_default_tab",)
    )
    ...
    default_tab_content = AliasProperty(
        get_def_tab_content, set_def_tab_content
    )
    ...
    def remove_widget(self, widget):
        ...
        if widget in (content, self._tab_layout):
            ...
            if not (self.do_default_tab and widget is self._default_tab):
                ...
                Logger.info(
                    "TabbedPanel: default tab! can't be removed.\n"
    ...
    def _update_tabs(self, *l):
        ...
        if pos_letter == "b" or pos_letter == "t":
            ...
            tab_layout.height = (
                tab_height
                + tab_layout.padding[1]
                + tab_layout.padding[3]
                + dp(2)
            )
            self_update_scrollview(scrl_v)
