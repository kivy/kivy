...
def getoutput(cmd, env=None):
    ...
    if p.returncode:  # if not returncode == 0
        print(
            "WARNING: A problem occurred while running {0} (code {1})\n".format(
                cmd, p.returncode
            )
        )
...
build_examples = (
    build_examples or os.environ.get("KIVY_BUILD_EXAMPLES", "0") == "1"
)
...
class KivyBuildExt(build_ext, object):
    ...
    def finalize_options(self):
        ...
        if hasattr(self, "parallel") and self.parallel is None:
            ...
            if self.parallel:
                print(
                    "Building extensions in parallel using {} cores".format(
                        self.parallel
                    )
                )
        ...
        global build_path
        if (
            self.build_lib is not None
            and exists(self.build_lib)
    ...
    def build_extensions(self):
        ...
        config_py += "CYTHON_BAD = {0}\n".format(
            repr(", ".join(map(str, CYTHON_UNSUPPORTED)))
        )
...
def _check_and_fix_sdl2_mixer(f_path):
    ...
    smpeg2_path = (
        "{}/Versions/A/Frameworks/smpeg2.framework" "/Versions/A/smpeg2"
    ).format(f_path)
...
cmdclass = {"build_factory": FactoryBuild, "build_ext": KivyBuildExt}
...
print(
    "Using this graphics system: {}".format(
...
if platform not in ("ios", "android") and (
    c_options["use_gstreamer"] in (None, True)
):
...
def determine_base_flags():
    ...
    if c_options["use_ios"]:
        ...
        flags["library_dirs"] += [
            join(environ.get("LOCALBASE", "/usr/local"), "lib")
        ]
        ...
        if v[2] >= "13.0.0":
            if "SDKROOT" in environ:
                ...
                print(
                    "Xcode detected at {}, and using OS X{} sdk".format(
...
graphics_dependencies = {
    ...
    "opengl.pyx": ["config.pxi", "common.pxi", "cgl.pxd", "gl_redirect.h"],
    "opengl_utils.pyx": ["opengl_utils_def.pxi", "cgl.pxd",],
    ...
    "scissor_instructions.pyx": ["config.pxi", "opcodes.pxi", "cgl.pxd"],
    "svg.pyx": [
        "config.pxi",
        "common.pxi",
        "texture.pxd",
...
if c_options["use_pangoft2"] in (None, True) and platform not in (
    ...
    if pango_flags and "libraries" in pango_flags:
        ...
        pango_depends = {
            "depends": ["lib/pango/pangoft2.pxi", "lib/pango/pangoft2.h"]
        }
...
if c_options["use_gstreamer"]:
    sources["lib/gstplayer/_gstplayer.pyx"] = merge(
        base_flags, gst_flags, {"depends": ["lib/gstplayer/_gstplayer.h"]}
    )
    ...
    for dep in deps:
        ...
        for path in paths_to_test:
            ...
            if exists(filename):
                ...
                break
        if not found:
            print("ERROR: Dependency for {} not resolved: {}".format(fn, dep))
...
def get_extensions_from_sources(sources):
    ...
    for pyx, flags in sources.items():
        ...
        f_depends = [
            x for x in depends if x.rsplit(".", 1)[-1] in ("c", "cpp", "m")
        ]
